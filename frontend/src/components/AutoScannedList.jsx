    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-400 text-sm">No scans available yet.</div>
        ) : (
          locations.map(loc => (
            <div 
              key={loc.id} 
              onClick={() => onLocationClick(loc.lat, loc.lon)}
              className={`p-3 rounded-lg border cursor-pointer hover:shadow-md transition-all ${getRiskColor(loc.risk_level)}`}
            >
              <div className="flex justify-between items-start mb-1">
                <span className="font-medium text-sm truncate pr-2">{loc.location_name}</span>
